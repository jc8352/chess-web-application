...
;(function () {
  ...
  var DEFAULT_DRAG_THROTTLE_RATE = 1 //20
  var ELLIPSIS = '…'
  ...
  var DEFAULT_APPEAR_SPEED = 200 //200
  var DEFAULT_MOVE_SPEED = 200 //200
  var DEFAULT_SNAPBACK_SPEED = 1 //60
  var DEFAULT_SNAP_SPEED = 1 //30
  ...
  CSS['highlight3'] = 'highlight3-5f37h'
  ...
  function constructor (containerElOrString, config) {
    ...
    function drawBoard () {
      ...
      // Add click listeners to squares (GPT)
      $board.find('.' + CSS.square).on('click', onSquareClick);
    }

    //GPT
    var selectedPiece = null;
    var selectedSourceSquare = null;
    var CLICK_DROP = false;

    function onSquareClick(event) {
      var square = $(event.currentTarget).attr('data-square');
      var piece = currentPosition[square];

      if (isFunction(config.onClickStart) && config.onClickStart(square, piece, selectedPiece) == false) return
      ...
      if (selectedPiece === null && piece) {
        ...
        movePiece(selectedSourceSquare, square, selectedPiece);
        //clearSelection();
      }
    }
    ...
    function highlightSquare(square, highlight) {
      if (highlight) {
        $('#' + squareElsIds[square]).addClass(CSS.highlight3);
      } else {
        $('#' + squareElsIds[square]).removeClass(CSS.highlight3);
      }
    }

    function clearSelection() {
      if (selectedSourceSquare) {
        highlightSquare(selectedSourceSquare, false);
      }
      selectedPiece = null;
    ...
    function movePiece(source, destination, piece) {
      var newPosition = deepCopy(currentPosition);
      delete newPosition[source];
      newPosition[destination] = piece;
      setCurrentPosition(newPosition);

      // Animate piece movement
      animateSquareToSquare(source, destination, piece, function() {
        drawPositionInstant();
        // Run custom onSnapEnd function if defined
        if (isFunction(config.onSnapEnd)) {
          config.onSnapEnd(source, destination, piece);
        }
      });
    }
    ...
    function beginDraggingPiece (source, piece, x, y) {
      ...
      isDragging = true
      draggedPiece = piece
      if (CLICK_DROP) draggedPieceSource = selectedSourceSquare
      else draggedPieceSource = source
    ...
    function stopDraggedPiece (location) {
      ...
      if (isFunction(config.onDrop)) {
        ...
        if (CLICK_DROP && result === 'snapback') {
          action='drop';
         //clearSelection();
        }
        else if (result === 'snapback' || result === 'trash') {
          action = result
        }
      }
